        ...
        <tr> <?php include ("blocks/lefttd.php"); ?> <!--Левый блок-->
		                   ...
		                   && isset($pagenumber) && isset ($publicdate)) //Проверка существования переменных
		 {
			...
			$result = mysql_query ("UPDATE books
			              SET authors = '$authors', name = '$name', placepublic = '$placepublic', 
						  pagenumber = '$pagenumber', publicdate = '$publicdate', udk = '$udk'
						  WHERE cipher = '$cipher'");
			if ($result == true)
			{
				echo "<p>Книга успешно обновлена</p>";
			}
			else
			{
				echo "<p>Книга не была обновлена!</p>";
			}
		 ...
		 { 
			 echo "<p>Вы ввели не всю информацию. Книга не может быть занесена в базу.</p>";
	     }
